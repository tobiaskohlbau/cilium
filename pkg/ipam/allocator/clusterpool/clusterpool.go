...
func (a *AllocatorOperator) Init() error {
	if option.Config.EnableIPv4 {
		...
		return errors.New("ClusterPool IPv4 CIDR can not be set if IPv4 is disabled")
	}
	...
	if option.Config.EnableIPv6 {
		...
		return errors.New("ClusterPool IPv6 CIDR can not be set if IPv6 is disabled")
	}
...
func (a *AllocatorOperator) Start(updater ipam.CiliumNodeGetterUpdater) (allocator.NodeEventHandler, error) {
	log.WithFields(logrus.Fields{
		"ipv4CIDRs": operatorOption.Config.ClusterPoolIPv4CIDR,
		"ipv6CIDRs": operatorOption.Config.ClusterPoolIPv6CIDR,
