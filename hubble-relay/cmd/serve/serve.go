...
func New(vp *viper.Viper) *cobra.Command {
	...
	flags.Bool(
		...
		false,
		"Disable (m)TLS for the server and allow clients to connect over plaintext.",
	)
	vp.BindPFlags(flags)
...
func buildClientTLSOption(vp *viper.Viper) (server.Option, error) {
	...
	case len(tlsHubbleServerCAPaths) > 0:
		...
		for _, path := range tlsHubbleServerCAPaths {
			...
			if ok := tlsHubbleServerCAs.AppendCertsFromPEM(certPEM); !ok {
				return nil, fmt.Errorf("cannot process cert '%s': %s", path, err)
			}
		}
	default:
		var err error
	...
	if tlsClientCertPath != "" && tlsClientKeyPath != "" { // mTLS
